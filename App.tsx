...
const App: React.FC = () => {
  ...
  const handleExportAllCsv = () => {
    const successfulResults = processResults.filter(r => r.status === 'success' || r.status === 'confirmed');
    if (successfulResults.length === 0 || !docType) return;
    
    const config = documentConfig[docType];
    const headerConfig = config.filter(c => c.enabled);
    const headers = ['ファイル名', 'ドキュメントタイプ', ...headerConfig.map(c => c.label)];
